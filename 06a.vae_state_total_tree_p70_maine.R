...
slow$NEWINVYR <- 0
for(i in 1:nrow(slow)){
  if(slow$PANEL_70[i] > 14 & slow$PANEL_70[i] <= 24){slow$NEWINVYR[i] <- 2020} else # slow: < 14 P70's
    if(slow$PANEL_70[i] > 24 & slow$PANEL_70[i] <= 34){slow$NEWINVYR[i] <- 2021} else # slow: < 14 P70's
      if(slow$PANEL_70[i] > 34 & slow$PANEL_70[i] <= 53){slow$NEWINVYR[i] <- 2022} else # fast: > 14 P70's
        if(slow$PANEL_70[i] > 53 & slow$PANEL_70[i] <= 70){slow$NEWINVYR[i] <- 2023} else # fast: > 14 P70's
          if(slow$PANEL_70[i] > 0 & slow$PANEL_70[i] <= 14){slow$NEWINVYR[i] <- 2024} # on time
}
...
plot(ESTIMATED_TOTAL,se / ESTIMATED_TOTAL * 100, col="red", pch = 19,
     ...
     #ylim = c(1.22,1.24),xlim = c(ESTIMATED_TOTAL - 100000000,ESTIMATED_TOTAL + 100000000))
     ylim = c(1.22,1.24),xlim = c(ESTIMATED_TOTAL - (0.0043*ESTIMATED_TOTAL),ESTIMATED_TOTAL + (0.0043*ESTIMATED_TOTAL)),
     xaxt = "n", main = "Simulated effect of variable annual effort on estimated totals and standard errors")
